...
def getSomaLoc(eis, neuron_index, elec_coords, adj_mat, thr=-3):
...
def plotEiStim(ei, stim_elecs, ax, coords, size_fac=-25,thr=-1,stim_sf=240,stim_mew=3,ei_color='b',stim_color='y',alpha=1):
    #Data
    #coords = loadCoords512()
    xs = coords[:,0]
    ys = coords[:,1]
    ...
    ax.scatter(eixs,eiys, facecolors=ei_color, edgecolors=ei_color, s=list(ei[goodInds]*size_fac),alpha=alpha)
    ##Plot stim elec
...
def getAxonFit(eis, neuron_index, elec_coords, adj_mat, thr=-1, pdeg = 4, distThr = 175):
